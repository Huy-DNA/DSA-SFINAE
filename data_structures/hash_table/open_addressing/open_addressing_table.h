...
template <
    typename Key_T, typename Value_T, typename Hasher_T = std::hash<Key_T>,
    std::size_t TABLE_SIZE = 101,
    typename = std::enable_if_t<std::is_trivially_copyable<Key_T>::value, void>,
    typename =
        std::enable_if_t<std::is_trivially_copyable<Value_T>::value, void>>
...
public:
  OpenAddressingTable(std::string method = "linear") : _probe_method{method} {
    if (!_valid_probe_method())
      throw std::runtime_error{"Invalid probing method."};
  }
  ...
  Value_T &operator[](const Key_T &key) {
    if (_probe_method == "linear") {
      ...
      throw std::runtime_error{
          "Unimplemented probe method"}; // To be implemented
  ...
  void insert(const Key_T &key, const Value_T &value) {
    if (_probe_method == "linear") {
      for (size_t round = 0; round < TABLE_SIZE; ++round) {
        ...
        if (_null_at(id)) {
          ...
          throw std::runtime_error{
              "OpenAddressingTable::insert() called on already inserted key."};
      }
    } else
      throw std::runtime_error{
          "Unimplemented probe method"}; // To be implemented
    ...
    throw std::runtime_error{
        "OpenAddressingTable::insert() called on full table."};
  }
...
private:
  ...
  size_t hash(const Key_T &key) const { return hasher(key) % TABLE_SIZE; }
  ...
  bool _null_at(size_t id) const { return _table[id].empty; }

  Key_T &_key_at(size_t id) { return _table[id].key; }

  Value_T &_value_at(size_t id) { return _table[id].value; }
