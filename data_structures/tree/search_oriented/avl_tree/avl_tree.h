...
public:
  ...
  ConstAVLBiIter minimum() const { return minimum_at_subtree(_root); }

  ConstAVLBiIter maximum() const { return maximum_at_subtree(_root); }
  ...
  ConstAVLBiIter predecessor(ConstAVLBiIter iter) {
    ...
    if (nptr == _NIL)
      ...
      while (nptr->p != _NIL && nptr->p->right != nptr)
        nptr = nptr->p;
  ...
  const Node *successor(const Node *iter) {
    ...
    if (nptr == _NIL)
      ...
      while (nptr->p != _NIL && nptr->p->left != nptr)
        nptr = nptr->p;
    return _create_ConstAVLBiIter(nptr->p);
  }
  ...
  void remove(const Elem_T &e) { remove(search(e)); }
  ...
  Elem_T remove(ConstAVLBiIter iter) {
    ...
    if (u->left == _NIL) {
      ...
      Node *v =
          _extract_Node(minimum_at_subtree(_create_ConstAVLBiIter(u->right)));
      if (v->p != u) {
        to_be_fixed = v->p;
        fixed_child = v->right;
  ...
  void print() const { _print_recur("", _root, false); }

  ~AVLTree() { _deallocate_node(_root); }

private:
  void _print_recur(const std::string &prefix, const Node *node,
                    bool isLeft) const {
    /* From:
  ...
  void _remove_fixup(Node *v, Node *u) {
    // v is the root of the smallest subtree that may be imbalanced due to
    // previous deletion u is the child of p that is balanced but there's a node
    // in its branch that was deleted
    bool shorter = true;
...
typename AVLTree<Elem_T, Comp_T, _SFINAE>::Node
    AVLTree<Elem_T, Comp_T, _SFINAE>::_sentinel{0, nullptr, nullptr, nullptr,
                                                0};

template <typename Elem_T, typename Comp_T, typename> class ConstAVLBiIter {
...
private:
  ConstAVLBiIter(const Node *node, const AVLTree<Elem_T, Comp_T> &owner)
      : _node{node}, _owner{owner} {}
  const Node *_node;
...
struct std::iterator_traits<ConstAVLBiIter<Elem_T, Comp_T>> {
  typedef const std::remove_cv_t<Elem_T>
      value_type; // cppreference does not specify what this does so...
  typedef const Elem_T
      *pointer; // cppreference does not specify what this does so...
  typedef const Elem_T
