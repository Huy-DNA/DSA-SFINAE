...
/* Require Elem_T to be a trivially copyable type */
template <typename Elem_T, typename Comp_T = std::less<const Elem_T &>,
          typename =
              std::enable_if_t<std::is_trivially_copyable<Elem_T>::value, void>>
class RedBlackTree {
...
public:
  ...
  ~RedBlackTree() { _deallocate_node(_root); }
  ...
  Elem_T remove(const Elem_T &e) { return remove(search(e)); }
  ...
  Elem_T remove(ConstRBBiIter iter) {
    ...
    if (node == _NIL)
      throw std::runtime_error{
          "Redblacktree::remove() called on null pointer."};
    ...
    if (node->left == _NIL) {
      ...
      Node *m =
          _extract_Node(minimum_at_subtree(_create_ConstRBBiIter(node->right)));
  ...
  ConstRBBiIter maximum() { return maximum_at_subtree(_root); }

  ConstRBBiIter minimum() { return minimum_at_subtree(_root); }
...
typename RedBlackTree<Elem_T, Comp_T, _SFINAE>::Node
    RedBlackTree<Elem_T, Comp_T, _SFINAE>::_sentinel = {
        {}, _NIL, _NIL, _NIL, BLACK};

template <typename Elem_T, typename Comp_T, typename> class ConstRBBiIter {
...
private:
  ConstRBBiIter(const Node *node, const RedBlackTree<Elem_T, Comp_T> &owner)
