...
public:
  ...
  LinkedListNonConstBiIter<Elem_T> search(
      const Elem_T &e,
      const std::function<bool(const Elem_T &, const Elem_T &)> &eq =
          [](const Elem_T &e1, const Elem_T &e2) { return e1 == e2; }) {
    Node *cur = _head;
    while (cur != _NIL && !eq(cur->data, e))
      cur = cur->next;
    if (cur == _NIL)
      cur = nullptr; // truly indicates that no element was found.
    return {cur};
  }

  template <typename T>
  LinkedListNonConstBiIter<Elem_T> search(
      const T &t, const std::function<bool(const Elem_T &, const T &)> &eq =
                      [](const Elem_T &e, const T &t) { return e == t; }) {
    Node *cur = _head;
  ...
  void insert_after(LinkedListNonConstBiIter<Elem_T> pos, const Elem_T &e) {
    if (pos._node == _NIL)
      throw std::runtime_error{
          "LinkedList::insert_after() called on invalid position."};
    // Note that if pos.node == _tail, because _tail is a reference to
    // pos.node->next->prev, there's no need to manually set it.
    ++_size;
  ...
  void insert_before(LinkedListNonConstBiIter<Elem_T> pos, const Elem_T &e) {
    if (pos._node == _NIL)
      throw std::runtime_error{
          "LinkedList::insert_before() called on invalid position."};
    // Note that if pos.node == _head, because _head is a reference to
    // pos.node->prev->next, there's no need to manually set it.
    ++_size;
  ...
  Elem_T remove(LinkedListNonConstBiIter<Elem_T> pos) {
    if (pos._node == _NIL)
      throw std::runtime_error{
          "LinkedList::remove() called on invalid position."};
    // Note that if pos.node == _tail, because _tail is a reference to
    // pos.node->next->prev, there's no need to manually set it. Note that if
    // pos.node == _head, because _head is a reference to pos.node->prev->next,
    // there's no need to manually set it.
    --_size;
  ...
  Elem_T pop_front() {
    if (_size == 0)
      throw std::runtime_error{"LinkedList::pop_front() called on empty list."};
    --_size;
  ...
  Elem_T pop_back() {
    if (_size == 0)
      throw std::runtime_error{"LinkedList::pop_back() called on empty list."};
    --_size;
  ...
  LinkedListNonConstBiIter<Elem_T> begin() { return {_head}; }

  /* Returns an iterator to the sentinel, which is past-one from the end. */
  LinkedListNonConstBiIter<Elem_T> end() { return {_NIL}; }

  /* Returns a reference to the last element. */
  Elem_T &back() {
    if (_size == 0)
      throw std::runtime_error{"LinkedList::back() called on empty list."};
    return _tail->data;
  }
  ...
  Elem_T &front() {
    if (_size == 0)
      throw std::runtime_error{"LinkedList::front() called on empty list."};
    return _head->data;
  }
...
private:
  // Technically, this is a circular linked list -- sentinel is the element that
  // closes the circle.
...
/*------------------------------------*/
template <typename Elem_T, typename> class LinkedListNonConstBiIter {
...
public:
  ...
  Elem_T &operator*() { return _node->data; }
  Elem_T *operator->() { return &(_node->data); }
