...
class MemoryPool {
  ...
  static constexpr size_t BUFFER_SIZE =
      1000; // Maximum number of elements that the memory pool can allocate
public:
  MemoryPool() { // Initialize memory pool
    _free = 0;
  ...
  Elem_T *allocate(const Elem_T &e = {}) {
    if (empty())
      throw std::runtime_error{"MemoryPool::allocate() called on empty pool."};
    --_size;
  ...
  bool empty() const { return _free == -1; }
