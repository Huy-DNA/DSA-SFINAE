...
template <typename Elem_T,
          typename =
              std::enable_if_t<std::is_trivially_copyable<Elem_T>::value, void>>
class CompactListNonConstBiIter;

/* Pointer-less Linked List implemented using a dynamic array -- array indices
 * act as pointers. It's also Compact -- meaning all elements always occupy the
 * leftmost slots of the array. Elem_T must be of TriviallyCopyableType. */
template <typename Elem_T,
          typename =
              std::enable_if_t<std::is_trivially_copyable<Elem_T>::value, void>>
class CompactList {
...
public:
  ...
  CompactListNonConstBiIter<Elem_T> sorted_search(
      const Elem_T &e,
      const std::function<bool(const Elem_T &, const Elem_T &)> &less =
          std::less<const Elem_T &>{}) {
    static std::mt19937_64 mt;
    std::uniform_int_distribution<size_t> gen{0, _size - 1};
    long long cur = _head;
    while (cur != _NIL && less(_buffer[cur].data, e)) {
      long long random_pos = gen(mt);
      if (_buffer[random_pos].data == e)
        return _create_iterator(random_pos);
      if (less(_buffer[random_pos].data, e) &&
          less(_buffer[cur].data, _buffer[random_pos].data))
        cur = random_pos;
      cur = _buffer[cur].next;
    }
    if (cur == _NIL || less(e, _buffer[cur].data))
      cur = _INVALID;
  ...
  void insert_after(CompactListNonConstBiIter<Elem_T> iter, const Elem_T &e) {
    if (iter.id == -1)
      throw std::runtime_error{
          "CompactList::insert_after() called on invalid position."};
    _insert_after(iter.id, e);
  }

  /* Insert `e` before `iter` */
  ...
  /* Insert `e` at the end of the list. */
  void push_back(const Elem_T &e) { _insert_after(_tail, e); }

  /* Insert `e` at the start of the list */
  void push_front(const Elem_T &e) { _insert_before(_head, e); }

  /* Remove and return the last element of the list */
  Elem_T pop_back() {
    if (empty())
      throw std::runtime_error{"CompactList::pop_back() called on empty list."};
  ...
  Elem_T pop_front() {
    if (empty())
      throw std::runtime_error{
          "CompactList::pop_front() called on empty list."};
    return _remove(_head);
  }

  /* Remove the element pointed to by `iter` from the list */
  void remove(CompactListNonConstBiIter<Elem_T> iter) {
    if (iter._id == _NIL or iter._id == _INVALID)
      throw std::runtime_error{
          "CompactList::remove() called on invalid position."};
    _remove(iter._id);
  }
  ...
  /* Return an iterator to the sentinel, which is past-one from the last element */
  CompactListNonConstBiIter<Elem_T> end() { return {_NIL, _buffer, _sentinel}; }

private:
  /* Remove element at position `pos` in the underlying array.
  ...
  /* Insert `e` that is `next` of the element at position `pos` in the
   * underlying array */
  ...
  /* Insert `e` that is `prev` of the element at position `pos` in the
   * underlying array */
  ...
  /* Work like _buffer[id], but if id == -1, returns a reference to the
   * sentinel */
  ...
  /* Halve the capacity of the underlying array */
  void _halve_buffer() {
    Node *sec_buffer = new Node[_capacity / 2];
    _copy(_buffer, _size, sec_buffer);
    delete[] _buffer;
  ...
  /* Copy `from_size` elements from `from_buffer` to `to_buffer` */
  static void _copy(Node *from_buffer, size_t from_size, Node *to_buffer) {
    memcpy(to_buffer, from_buffer, from_size * sizeof(Node));
  }

  /*Check whether the underlying array have reached its capacity.*/
  bool _full() const { return _size == _capacity; }

  /*Check whether the current number of elements in the array is less than a
   * quarter of its capacity.*/
  bool _can_halve() const { return _capacity > 1 && _size <= _capacity / 4; }

  size_t _size = 0;     // Number of elements in the list
  size_t _capacity = 1; // Capacity of the underlying array
  Node *_buffer =
      new Node[_capacity]; // Underlying array for holding list's elements

  Node _sentinel = {
      {}, -1, -1}; // _sentinel.next == _head, _sentinel.prev == _tail
...
/*-------------------------------------*/
template <typename Elem_T, typename> class CompactListNonConstBiIter {
...
public:
  CompactListNonConstBiIter(const CompactListNonConstBiIter &other)
      : _id{other._id}, _list_buffer{other._list_buffer},
        _list_sentinel{other._list_sentinel} {}

  bool operator==(CompactListNonConstBiIter other) const {
    return (other._id == _id) && (other._list_buffer == _list_buffer) &&
           (&other._list_sentinel == &_list_sentinel);
  }
  ...
  Elem_T &operator*() {
    ...
    case _NIL:
      throw std::runtime_error{"Dereferencing sentinel."};
    default:
      return _list_buffer[_id].data;
    }
  }
  ...
  CompactListNonConstBiIter &operator++() {
    ...
    case _INVALID:
      throw std::runtime_error{"Invalid iterator."};
    case _NIL:
      _id = _list_sentinel.next;
      break;
    default:
      _id = _list_buffer[_id].next;
    }
    return *this;
  }
  ...
  CompactListNonConstBiIter &operator--() {
    ...
    case _INVALID:
      throw std::runtime_error{"Invalid iterator."};
    case _NIL:
      _id = _list_sentinel.prev;
      break;
    default:
      _id = _list_buffer[_id].prev;
    }
    return *this;
  }

private:
  CompactListNonConstBiIter(long long i, Node *buffer, Node &sentinel)
      : _id{i}, _list_buffer{buffer}, _list_sentinel{sentinel} {}
  long long _id;
  Node *_list_buffer;
  Node &_list_sentinel;
};
...
struct std::iterator_traits<CompactListNonConstBiIter<Elem_T>> {
  typedef std::remove_cv_t<Elem_T>
      value_type;          // cppreference does not specify what this does so...
  typedef Elem_T *pointer; // cppreference does not specify what this does so...
  typedef Elem_T
      &reference; // cppreference does not specify what this does so...
  typedef std::bidirectional_iterator_tag
      iterator_category; // I know what this does!!!
};
#endif
